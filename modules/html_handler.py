...
# Ensure 'BeautifulSoup' is imported or available in this file's imports

async def html_to_txt_handler(client: Client, message: Message):
    """
    Handles the /h2t command: downloads an HTML file reply, 
    extracts clean text, and uploads the resulting TXT file.
    """
    if not message.reply_to_message or not message.reply_to_message.document:
        await message.reply_text("Please reply to an **HTML file** (`.html`) to convert it to a `.txt` file.")
        return

    doc = message.reply_to_message.document
    if not doc.file_name.lower().endswith('.html'):
        await message.reply_text("Invalid file type. Please reply only to a document ending with **`.html`**.")
        return

    # Create a unique temporary directory for safety
    temp_dir = f"./downloads/h2t_{message.chat.id}_{message.id}"
    os.makedirs(temp_dir, exist_ok=True)
    
    html_file_path = os.path.join(temp_dir, doc.file_name)
    
    try:
        # 1. Download the HTML file
        await message.reply_to_message.download(file_name=html_file_path)
        
        editable = await message.reply_text("⏳ Converting HTML to TXT...")
        
        # 2. Read, Parse, and Extract Text
        with open(html_file_path, 'r', encoding='utf-8') as f:
            html_content = f.read()

        soup = BeautifulSoup(html_content, 'html.parser')
        # Extract readable text content, removing script, style tags, etc.
        text_content = soup.get_text(separator='\n', strip=True)

        # 3. Create the new TXT file
        base_name = os.path.splitext(doc.file_name)
        output_filename = os.path.join(temp_dir, f"{base_name}.txt")
        
        with open(output_filename, 'w', encoding='utf-8') as f:
            f.write(text_content)

        # 4. Upload the new TXT file
        await client.send_document(
            chat_id=message.chat.id,
            document=output_filename,
            caption=f"✅ **HTML to TXT conversion successful!**\n\nFile: `{base_name}.txt`"
        )
...
async def html_to_txt_handler(client: Client, message: Message):
    """
    Handles the /h2t command: user replies to an HTML file,
    bot converts it to TXT and sends it back.
    """
    if not message.reply_to_message or not message.reply_to_message.document:
        await message.reply_text("⚠️ Please reply to an **HTML file** (`.html`) to convert it to a `.txt` file.")
        return

    doc = message.reply_to_message.document
    if not doc.file_name.lower().endswith('.html'):
        await message.reply_text("❌ Invalid file type. Only `.html` files are supported.")
        return
    ...
    try:
        ...
        await message.reply_to_message.download(file_name=html_file_path)
        editable = await message.reply_text("⏳ Converting HTML → TXT ...")
        ...
        # 4. Send back TXT file
        await client.send_document(
            chat_id=message.chat.id,
            document=output_filename,
            caption=f"✅ HTML → TXT conversion successful!\nFile: `{base_name}.txt`"
        )
